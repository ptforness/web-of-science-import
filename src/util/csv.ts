 ...
 */
export async function read(filepath: string, options: Options = defaultOptions): Promise<unknown[]> {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const values: unknown[] = rows[i].split(options.delimiter);
    json[i - 1] = values
 ...
 */
function _parseIfStringified(json: unknown[] | string): unknown[] {
 ...
 */
function _valuesToRecord(values: unknown[], delimiter: string): string {
